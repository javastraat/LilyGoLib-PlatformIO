...
static void hw_device_poll(lv_timer_t *t)
{
    ...
    if (params.battery_voltage < 3300 && params.usb_voltage == 0) {
        printf("Low battery voltage: %hu mV USB Voltage: %hu mV\n", params.battery_voltage, params.usb_voltage);
...
void setupGui()
{
    ...
    extern app_t ui_universalmesh_main;
    ...
    // #endif
    create_app(panel, "Test Screen", &img_test, &ui_factory_main);
    create_app(panel, "UniversalMesh", &img_radio, &ui_universalmesh_main);
